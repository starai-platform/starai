...
type PaymentPackage = {
  ...
  effective_compute_credits: number;
  credits_mode: "manual" | "formula";
  badge: string;
  is_enabled: boolean;
  sort_order: number;
  updated_at: string;
...
const EMPTY = { name: "", amount: 10, compute_credits: "", badge: "", is_enabled: true, sort_order: 10 };
...
export default function PaymentPackagesPage() {
  ...
  const openEdit = (item: PaymentPackage) => {
    setEditing(item);
    setForm({ name: item.name, amount: item.amount, compute_credits: item.compute_credits == null ? "" : String(item.compute_credits), badge: item.badge || "", is_enabled: item.is_enabled, sort_order: item.sort_order });
    setMessage("");
  };
  ...
  const save = async () => {
    ...
    try {
      await adminApi(editing?.public_id ? `/payment-packages/${editing.public_id}` : "/payment-packages", {
        ...
        body: JSON.stringify({ ...form, compute_credits: form.compute_credits.trim() === "" ? null : Number(form.compute_credits) }),
      });
