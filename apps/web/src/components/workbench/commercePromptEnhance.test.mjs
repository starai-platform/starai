...
test("commerce workspace enhances prompts according to the selected creative mode", () => {
  ...
  assert.match(source, /clientScenePrompt\(selectedSceneMeta\.code, selectedSceneMeta\.label, generationType, creativeMode\)/);
  assert.match(source, /当前为自由创作：保留用户明确要求/);
  assert.match(source, /当前为精准还原：商品、Logo、包装和用户锁定元素以参考图为准/);
  assert.match(source, /AI主动补全商品材质、技术、功效、卖点/);
  assert.match(source, /详情页模块由 AI 按内容规划，最多/);
...
test("image workspaces share a readable inline reference area", () => {
  ...
  assert.match(source, /一句话描述想要的效果，或只上传图片让 AI 自由发挥/);
  assert.match(source, /一句话描述想做的带货视频，也可以只上传素材让 AI 自动完成脚本与成片/);
